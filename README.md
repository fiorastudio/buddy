...
Buddy runs inside your existing IDE session. No separate API calls, no additional billing, no new account.

**Static overhead (loaded every turn, cached after turn 1):**

MCP tool descriptions (8 tools, 3 resources) + companion system prompt add ~2,000 tokens to your context. These are included in input tokens on every turn, but Claude Code enables [prompt caching](https://docs.anthropic.com/en/docs/build-with-claude/prompt-caching) automatically — cached tokens are charged at **10% of normal input price** after the first turn.

| | Turn 1 (cache write) | Turns 2+ (cache read) | 10-turn session total |
|---|---|---|---|
| Buddy overhead | ~2,000 tokens @ 1.25x | ~2,000 tokens @ 0.1x | ~$0.00001 (Sonnet) |
...
- **Backseat mode**, 15 calls/session: ~$0.002–$0.005 per session
- **Skillcoach mode**, 15 calls/session: ~$0.005–$0.010 per session
- **Both mode**, 15 calls/session: ~$0.007–$0.012 per session
- **Buddy's static overhead** (tool definitions in context): ~$0.00001 per session after caching

For comparison, a single complex coding prompt ("refactor this module") costs $0.05–$0.15. Buddy is <5% of typical session cost.
